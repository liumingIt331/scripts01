/*
转载至：执意ZhiYi-N
原文地址：https://raw.githubusercontent.com/ZhiYi-N/Private-Script/master/Scripts/hotsoon.js
*/
...
let hsCurrentAccIndex = $.getdata('hsCurrentAccIndex') || "1"; // 默认账号一
if (isGetCookie) {
    GetCookie(hsCurrentAccIndex);
    $.done()
}
if ($.isNode()) {
//sign
...
function GetCookie(hsCurrentAccIndex) {
    ...
    if($request&&$request.url.indexOf("hotsoon"&&"sign_in_detail")>=0) {
        ...
        if (hotsoonsignheader) $.setdata(hotsoonsignheader,`hotsoonsignheader${suffix}`)
        $.log(`[${jsname}${suffix}] 获取sign请求: 成功,hotsoonsignheader: ${hotsoonsignheader}`)
        $.msg(`[${jsname}${suffix}]获取hotsoonsignheader: 成功🎉`, ``)
        const hotsoonsignkey = JSON.stringify($request.headers)
        if(hotsoonsignkey)        $.setdata(hotsoonsignkey,`hotsoonsignkey${suffix}`)
        $.log(`[${jsname}${suffix}] 获取sign请求: 成功,hotsoonsignkey: ${hotsoonsignkey}`)
        $.msg(`[${jsname}${suffix}]获取hotsoonsignkey: 成功🎉`, ``)
    }
    if($request&&$request.url.indexOf('hotsoon'&&"daily_read")>=0) {
        const hotsoonreadheader = $request.url.split(`?`)[1]
        if (hotsoonreadheader) $.setdata(hotsoonreadheader,`hotsoonreadheader${suffix}`)
        $.log(`[${jsname}${suffix}] 获取read请求: 成功,hotsoonreadheader: ${hotsoonreadheader}`)
        $.msg(`[${jsname}${suffix}]获取hotsoonreadheader: 成功🎉`, ``)
        const hotsoonreadkey = JSON.stringify($request.headers)
        if(hotsoonreadkey)        $.setdata(hotsoonreadkey,`hotsoonreadkey${suffix}`)
        $.log(`[${jsname}${suffix}] 获取read请求: 成功,readkey: ${hotsoonreadkey}`)
        $.msg(`[${jsname}${suffix}]获取hotsoonreadkey: 成功🎉`, ``)
    }
    if($request&&$request.url.indexOf('hotsoon' && "draw_excitation_ad")>=0) {
        const hotsoonadheader = $request.url.split(`?`)[1]
        if (hotsoonadheader) $.setdata(hotsoonadheader,`hotsoonadheader${suffix}`)
        $.log(`[${jsname}${suffix}] 获取AD请求: 成功,hotsoonadheader: ${hotsoonadheader}`)
        $.msg(`[${jsname}${suffix}]获取hotsoonadheader: 成功🎉`, ``)
        const hotsoonadkey = JSON.stringify($request.headers)
        if(hotsoonadkey)        $.setdata(hotsoonadkey,`hotsoonadkey${suffix}`)
        $.log(`[${jsname}${suffix}] 获取AD请求: 成功,hotsoonadkey: ${hotsoonadkey}`)
        $.msg(`[${jsname}${suffix}]获取hotsoonadkey: 成功🎉`, ``)
    }
}
//签到
function sign_in() {
