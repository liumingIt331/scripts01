/*
转载 https://raw.githubusercontent.com/adwktt/adwktt/master/BBB.js
*/
...
[rewrite_local]
#步步宝多账号
https://bububao.duoshoutuan.com/user/profile url script-request-header https://raw.githubusercontent.com/liumingIt331/scripts01/master/BBBMultiple.js

[task_local]
0 8-23/2 * * * https://raw.githubusercontent.com/liumingIt331/scripts01/master/BBBMultiple.js, tag=步步宝多账号,

loon
[Script]
http-request https://bububao.duoshoutuan.com/user/profile script-path= https://raw.githubusercontent.com/liumingIt331/scripts01/master/BBBMultiple.js, timeout=10, tag= 步步宝多账号

cron "0 8-23/2 * * *" script-path= https://raw.githubusercontent.com/liumingIt331/scripts01/master/BBBMultiple.js, tag= 步步宝多账号
...
if (typeof $request !== 'undefined') {
    if ($request && $request.method != `OPTIONS` && $request.url.indexOf('user/profile') != -1) {
        let index = bbbCurrentAccIndex > 1 ? bbbCurrentAccIndex : '';
...
function tixian(username, money) {
    ...
    if (tixianJsonInfo.time && tixianJsonInfo.money && (new Date(tixianJsonInfo.time).toDateString() === new Date().toDateString()) ) {
        notice += `🎉提现情况(${tixianJsonInfo.money}): 今日已提现完成\n`
        return;
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(tixianInfo, async (error, response, data) => {
            ...
            if (response.statusCode == 200) {
                ...
                notice += `🎉提现情况(${money}): ${respInfo.msg}\n`

                var timestamp = new Date().getTime();
                tixianJsonInfo.time = timestamp
                tixianJsonInfo.money = money
                $.getdata(JSON.stringify(tixianJsonInfo), `tixian${$.index}`)
            }
            resolve()
        })
    })
