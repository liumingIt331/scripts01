...
转载ziye大佬的代码 https://github.com/ziye12/JavaScript
修改为多账号的版本 结合boxjs  https://raw.githubusercontent.com/liumingIt331/scripts01/master/boxjs/boxjs.json
请在boxjs填写账号的序号依次获取ck
...
#企鹅读书获取更新body
https:\/\/mqqapi\.reader\.qq\.com\/log\/v4\/mqq\/track url script-request-body https://raw.githubusercontent.com/liumingIt331/scripts01/master/quantumultX/qqreads.js

#企鹅读书获取时长cookie
https:\/\/mqqapi\.reader\.qq\.com\/mqq\/addReadTimeWithBid? url script-request-header https://raw.githubusercontent.com/liumingIt331/scripts01/master/quantumultX/qqreads.js
...
const logs = $.getdata('logs') ? 1 : 0; // 0为关闭日志，1为开启
const notifyInterval = $.getdata('notifyInterval');
// 0为关闭通知，1为所有通知，2为宝箱领取成功通知，3为宝箱每15次通知一次

let currentAccIndex = $.getdata('qqIndex') || "1"; // 默认账号一
let cookiesJson = $.getdata('cookiesQQ');
...
if ($.isNode()) {
    ...
    if (cookiesJson) {
        ...
        for (let o in cookiesJsonObj) {
            ...
            qqreadbdArr.push(value.qqreadbd);
            qqreadtimeurlArr.push(value.qqreadtimeurl);
            qqreadtimehdArr.push(value.qqreadtimehd);
        }
    }
}
...
function GetCookie(accIndex) {
    ...
    let cookieObj = cookiesJsonObj[accIndex] || {"qqreadtimeurl": "", "qqreadtimehd": "", "qqreadbd": ""};
