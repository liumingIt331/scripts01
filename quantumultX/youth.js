...
if ($.isNode()) {
    ...
    cookieArr.push($.getdata('youthheader_zq1'));
    readArr.push($.getdata('read_zq1'));
    timeArr.push($.getdata('readtime_zq1'))
    // 根据boxjs中设置的额外账号数，添加存在的账号数据进行任务处理
    let count = ($.getval('totalAcc') || '1') - 0;
    for (let i = 2; i <= count; i++) {
        cookieArr.push($.getdata(`youthheader_zq${i}`));
...
function GetCookie() {
    if ($request && $request.method != `OPTIONS` && $request.url.match(/\/NewTaskIos\/getTaskList/)) {
        ...
        $.log(`${$.name + $.idx}获取Cookie: 成功, signheaderVal: $}`);
        ...
        $.log(`${$.name + $.idx}获取阅读时长: 成功, timebodyVal: ${timebodyVal}`);
